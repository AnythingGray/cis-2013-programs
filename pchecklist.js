...
var passwordCheckOld = function () 
{
    ...
    stringOutput = "";
 //This if statement makes it so that if the length of the password is less than 8 it returns "This password is too short."
   if (stringPassword.length<8)
   {
      stringOutput = "This password is too short.";
   }
   //If the password is longer than 8, it goes on to check if the password is listed as part of the array
   else
   {
   
      for (var f=0; f < arrayBadPasswords.length; f++)
      {
         //If the password is listed any point on the array, it returns "This is a bad password" in string output
         if (stringPassword === arrayBadPasswords[f])
         {
            stringOutput = "This is a bad password.";
         }
      }
   }
   //If the password triggered the IF statements, and the output is NOT equal to " " than the string output is given
   if (stringOutput != "")
   {
      $("output").value= stringOutput;
   }
   //If the password hasn't triggered the IF statements than the password has passed the checker, and the output is "This is a good password."
   else
   {
      $("output").value= "This is a good password.";
   }
};
